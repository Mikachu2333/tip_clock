...
| 提示音        | 内置 start / end / special / none 四种提示音，也支持自定义 WAV、FLAC、MP3 文件 |
...
5. 自定义 WAV、FLAC、MP3 文件放在 EXE 同目录下，三种格式都可省略扩展名。名称相同时按 WAV → FLAC → MP3 的顺序选择；写明扩展名则使用指定文件。不允许绝对路径或子目录。
6. 程序优先在 EXE 目录保存配置；若该目录不可写，则自动使用 `%LOCALAPPDATA%\TipClock\config.toml`。
...
从 [Releases](https://github.com/Mikachu2333/tip_clock/releases) 下载 `tip_clock.exe`，放到任意文件夹后运行。首次启动会自动创建 `config.toml`。
...
#       end     = 内置结束提示音
#       special = 内置特别提示音
#       custom  = 播放自定义 WAV / FLAC / MP3（需设置 custom_file）
#       none    = 静音，仅弹窗
...
| `end`     | 内置结束提示音                                                  |
| `special` | 内置特别提示音                                                  |
| `custom`  | 播放 EXE 目录下的 WAV、FLAC 或 MP3 文件（需设置 `custom_file`） |
| `none`    | 静音，仅弹出窗口                                                |
