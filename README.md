...
| 提示音        | 内置 start / end / special / none 四种提示音，也支持自定义 WAV 文件 |
...
5. 自定义 WAV 文件放在 EXE 同目录下，配置中填写文件名；可省略 `.wav` 后缀，不允许绝对路径或子目录。
...
#       end     = 内置结束提示音
#       special = 内置特别提示音
#       custom  = 播放自定义 WAV（需设置 custom_file）
#       none    = 静音，仅弹窗
