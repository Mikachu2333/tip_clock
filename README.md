...
2. 提醒时间格式为 `HH:MM:SS`（24 小时制），个位数需补零，例如 `08:00:00`。
3. `display_time` 和 `[[schedule]]` 支持运行时热重载并自动去重；`hotkey_*`、`auto_start`、`volume` 修改后需重启。颜色、透明度和窗口位置请通过程序界面修改。
4. WAV、FLAC、MP3 文件放在 `config.toml` 所在目录，三种格式都可省略扩展名。名称相同时按 WAV → FLAC → MP3 的顺序选择；写明扩展名则使用指定文件。不允许绝对路径或子目录。
5. 程序优先在 EXE 目录保存配置；若该目录不可写，则自动使用 `%LOCALAPPDATA%\TipClock\config.toml`。首次创建配置时，会在同一目录释放示例音频 `demo.mp3`。
6. `volume` 仅控制 Tip Clock 自身的音频输出，不会修改 Windows 系统音量。
...
相同 `time` 的条目属于一个“提醒组”。例如同一秒包含一个静音条目以及 `audio = "A"`、`audio = "B"`：

- 时钟只弹出一次，静音条目不会额外弹窗。
- A、B 按配置文件中的先后顺序排队播放，不会同时播放或互相中断。
- 完全相同的 `time + audio` 会自动去重；相同时间但不同 audio 会保留。
- “跳过下一组提醒”会跳过该时刻的整个提醒组；暂停状态也不会补播该组。
